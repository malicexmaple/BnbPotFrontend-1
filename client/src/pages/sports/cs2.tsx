// Counter Strike Games List Page
...
export default function CounterStrike() {
  ...
  const cs2Markets = allMarkets?.filter(m => m.league === 'CS2' || m.league === 'Counter-Strike') || [];
  const games = groupMarketsByGame(cs2Markets);
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <img 
                  src="/sport-icons/cs2.png" 
                  alt="Counter Strike" 
                  className="w-10 h-10 object-contain brightness-0 invert"
                />
                <h2 className="text-3xl font-bold text-foreground font-sohne">
                  COUNTER STRIKE <span className="text-primary ml-1">MATCHES</span>
                </h2>
              </div>
            </div>
            ...
            {marketsLoading ? (
              ...
              <div className="text-center py-12 bg-card rounded-lg border border-card-border">
                <p className="text-muted-foreground text-lg">No active Counter Strike matches</p>
