...
import { Eye, EyeOff, Clock, Hand, Zap, RotateCcw } from "lucide-react";
...
export function SportVisibilityControls() {
  ...
  const resetLeagueMutation = useMutation({
    mutationFn: async ({ leagueId }: { leagueId: string }) => {
      return await apiRequest('DELETE', `/api/admin/visibility/league/${leagueId}/reset`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/visibility-settings'] });
      toast({ title: "League reset to automatic mode" });
    },
  });
  ...
  return (
    <Card className="bg-card/50 backdrop-blur-sm border-card-border" data-testid="card-visibility-controls">
      <CardHeader>
        <CardTitle>Sport & League Visibility</CardTitle>
        <CardDescription>
          Control which sports and leagues appear in the sidebar and markets. Leagues without events in the next 2 weeks are auto-hidden unless manually overridden. Use the reset button to return a league to automatic mode.
      ...
      <CardContent className="space-y-4">
        {displayedSports.map((sport) => {
          ...
          const displayedLeagues = sport.leagues;
                        ...
                        <div className="flex items-center gap-2">
                          {status.manualOverride && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => resetLeagueMutation.mutate({ leagueId: league.id })}
                              disabled={resetLeagueMutation.isPending}
                              data-testid={`button-reset-league-${league.id}`}
