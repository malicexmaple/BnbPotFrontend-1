...
async function fetchAllBaseballLeagues() {
  try {
    ...
    const baseballLeagues = data.leagues
      .filter((league: ApiLeague) => league.strSport === "Baseball")
      .map((league: ApiLeague) => ({
        id: league.idLeague,
        name: league.strLeague,
      }));

    console.log(`Found ${baseballLeagues.length} baseball leagues\n`);
    ...
    console.log(`\n\nGenerated ${leaguesWithBadges.length} baseball leagues with details\n`);
    ...
    console.error("Error fetching baseball leagues:", error);
  }
}

fetchAllBaseballLeagues();
